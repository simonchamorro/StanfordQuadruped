    ...
    for steps in range(timesteps):
        ...
        if sim_time_elapsed - last_control_update > config.dt:
            ...
            if use_policy:
                ...
                controller.send_action(state, command, use_offset=True)
            else:
                controller.run(state, command)
...
if __name__ == "__main__":
    ...
    print(last_models_path)

    main(default_velocity=np.array([TARGET[0], TARGET[1]]), policy_path=last_models_path, use_policy=True)
