...
class Kalman:
    ...
    def _prediction(self, X_hat_t_1,P_t_1):
        ...
        X_hat_t=F_t.dot(X_hat_t_1)+(self.B_t.dot(self.U_t).reshape(self.B_t.shape[0],-1) )
    ...
    def _update(self, X_hat_t, P_t, Z_t):
        ...
        K_prime=P_t.dot(self.H_t.transpose()).dot( np.linalg.inv ( self.H_t.dot(P_t).dot(H_t.transpose()) + self.R_t ) )  
